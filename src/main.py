...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == "--help":
        ...
        print(f"  REQUEST_TIMEOUT - Legacy timeout in seconds (default: 90)")
        print("")
        print("Fine-grained timeout settings (recommended):")
        print(f"  CONNECT_TIMEOUT - Connection timeout in seconds (default: 10)")
        print(f"  READ_TIMEOUT - Read timeout in seconds (default: 600)")
        print(f"  WRITE_TIMEOUT - Write timeout in seconds (default: 10)")
        print(f"  POOL_TIMEOUT - Connection pool timeout in seconds (default: 10)")
        print("")
        print("Model mapping:")
    ...
    print(f"   Timeouts: connect={config.connect_timeout}s, read={config.read_timeout}s, write={config.write_timeout}s, pool={config.pool_timeout}s")
